...
class SoundSpacesSpeechDataset(Dataset):
    def __init__(
        ...
        if remove_oov:
            ...
            tgt_file = os.path.join(
                "data/soundspaces_speech/remove_oov", split + ".pkl"
            )
            if os.path.exists(tgt_file):
                ...
                for file in tqdm(self.files):
                    ...
                    speaker_in_view = (
                        np.sum(
                            np.concatenate(
                                [x == 41 for x in file_data["semantic"]], axis=1
                            )
                        )
                        >= 50
                    )
            ...
            print(
                f"Number of files after removing out-of-view cases: {len(self.files)}"
            )
    ...
    def __getitem__(self, item):
        ...
        visual_sensors = []
        if self.use_rgb:
            sample["rgb"] = to_tensor(
                np.concatenate([x / 255.0 for x in data["rgb"]], axis=1)
            ).permute(2, 0, 1)
            visual_sensors.append("rgb")
        if self.use_depth:
            sample["depth"] = to_tensor(
                np.concatenate(data["depth"], axis=1)
            ).unsqueeze(0)
        ...
        if len(visual_sensors) > 0:
            if self.split == "train":
                ...
                for visual_sensor in visual_sensors:
                    if width_shift is None:
                        width_shift = np.random.randint(
                            0, sample[visual_sensor].shape[-1]
                        )
                    sample[visual_sensor] = torch.roll(
                        sample[visual_sensor], width_shift, dims=-1
                    )
            ...
            if self.limited_fov:
                    ...
                    for visual_sensor in visual_sensors:
                        if offset is None:
                            offset = np.random.randint(
                                0, sample[visual_sensor].shape[-1] - 256
                            )
                        sample[visual_sensor] = sample[visual_sensor][
                            :, :, offset : offset + 256
                        ]
                else:
                    for visual_sensor in visual_sensors:
                ...
                for visual_sensor in visual_sensors:
                    sample[visual_sensor] = torchvision.transforms.Resize((192, 576))(
                        sample[visual_sensor]
                    )
    ...
    def process_audio(self, source_audio, receiver_audio):
        ...
        if waveform_length > source_audio.shape[0]:
            receiver_audio = np.pad(
                receiver_audio, (0, max(0, waveform_length - receiver_audio.shape[0]))
            )
            source_audio = np.pad(
                source_audio, (0, waveform_length - source_audio.shape[0])
            )
        ...
        if self.split != "train":
            ...
            start_index = (
                np.random.randint(0, source_audio.shape[0] - waveform_length)
                if source_audio.shape[0] != waveform_length
                else 0
            )
