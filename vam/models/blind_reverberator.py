...
class BlindReverberator(BaseAVModel):
    ...
    def synthesize_ir(rt60, drr):
        rirs = []
        for rt60_est, drr_est in zip(
            rt60.squeeze().cpu().numpy(), drr.squeeze().cpu().numpy()
        ):
            Fs = 16000
            RT = rt60_est
            DRR = drr_est
            ...
            engDir = engRev * 10 ** (DRR / 10)
            # Direct energy, dictated by rev. energy and DRR
            presDir = np.sqrt(
                engDir / sum(dirPulse**2)
            )  # Pressure amplitude for the peak of the direct sound

            rir = np.concatenate(
                [
                    np.zeros(int(0.002 * Fs)),
                    presDir * np.transpose(dirPulse),
                    np.zeros(round(ITDG * Fs)),
                    decayingNoise,
                ]
            )  # build the RIR
            rir = (
                rir + np.random.randn(len(rir)) / 1000
            )  # Add some noise (this should be a parameter, not hard coded)
    ...
    def compute_different_source(self, batch):
        ...
        content, _ = librosa.load(
            "data/LibriSpeech/dev-clean/1272/128104/1272-128104-0001.flac", sr=16000
        )
    ...
    def acoustic_match(self, batch, batch_idx, phase=None):
        ...
        if RT60_ESTIMATOR is None:
            RT60_ESTIMATOR = load_rt60_estimator(self.device, self.args.model_dir)

        pred_rt60 = self.estimate_rt60(
            RT60_ESTIMATOR, self.compute_different_source(batch)
        )
