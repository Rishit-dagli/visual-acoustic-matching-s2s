...
def load_dereverberator(
    device, use_vida=False, path="./.cache/see2sound/avitar/dereverb/vida.pth"
):
    if use_vida:
        from vam.models.vida import VIDA
        from vam.trainer import parser

        pretrained_weights = os.path.join(os.path.dirname(path), "dereverb", "vida.pth")
        ...
        pretrained_weights = os.path.join(
            os.path.dirname(path), "dereverb", "avitar.pth"
        )
        state_dict = torch.load(pretrained_weights, map_location="cpu")
        default_args = parser.parse_args("")
...
class AViTAr(BaseAVModel):
    def __init__(self, args):
        ...
        if args.use_cnn:
            ...
            layers = list(
                torchvision.models.resnet18(pretrained=args.pretrained_cnn).children()
            )[1:-2]
            self.cnn = nn.Sequential(conv1, *layers)

        crossmodal_dropout_p = input_dropout_p = feed_forward_dropout_p = (
            attention_dropout_p
        ) = conv_dropout_p = (0 if args.no_dropout else args.dropout)
        crossmodal_dropout_p = (
            args.crossmodal_dropout_p
            if args.crossmodal_dropout_p != -1
            else crossmodal_dropout_p
        )
        input_dim = (
            257
            if not self.args.encode_wav
            else args.ngf * (2 ** len(self.args.encoder_ratios.split(",")))
        )
        self.conformer = Conformer(
            ...
            decode_wav=args.decode_wav,
            encode_wav=args.encode_wav,
            encoder_ratios=args.encoder_ratios,
            decoder_ratios=args.decoder_ratios,
            encoder_residual_layers=args.encoder_residual_layers,
            decoder_residual_layers=args.decoder_residual_layers,
            ngf=args.ngf,
            use_visual_pe=self.args.use_visual_pe,
        ...
        self.speech_metric_names = {"stoi", "sisdr", "bsseval", "mosnet"}.intersection(
            self.metrics
        )
    ...
    def get_visual_feat(self, batch):
        ...
        if self.args.use_cnn:
            ...
            if self.args.adaptive_pool:
                tgt_shape = (
                    (1, 1)
                    if self.args.mean_pool
                    else (img_feat.shape[-2] // 2, img_feat.shape[-1] // 2)
                )
                img_feat = F.adaptive_avg_pool2d(img_feat, tgt_shape)
            img_feat = img_feat.reshape(img_feat.size(0), img_feat.size(1), -1).permute(
                0, 2, 1
            )
        else:
            img_feat = None
    ...
    def acoustic_match(self, batch, batch_idx, phase=None):
        ...
        if self.args.dereverb_avspeech:
            dereverb_avspeech(
                batch,
                self.args.encode_wav,
                self.device,
                use_vida=self.args.use_vida,
...
def dereverb_avspeech(
    batch, encode_wav, device, use_vida=False, path="./.cache/see2sound/avitar/avspeech"
):
    with torch.no_grad():
        global DEREVERBERATOR
        if DEREVERBERATOR is None:
            DEREVERBERATOR = load_dereverberator(device, use_vida, path=path)
...
def convolve_random_rir(batch):
    ...
    for i in range(batch["src_wav"].shape[0]):
        ...
        outputs.append(
            torch.nn.functional.conv1d(audio, rir, padding=rir.shape[-1] - 1)
        )
...
def apply_audio_data_augmentation(args, batch, phase):
    ...
    transforms = []
    transforms.append(
        AddColoredNoise(p=1, min_snr_in_db=args.min_snr, max_snr_in_db=args.max_snr)
    )
    ...
    if len(transforms) != 0:
        ...
        batch["src_wav"] = apply_augmentation(
            batch["src_wav"].unsqueeze(1), sample_rate=16000
        ).squeeze(1)
        assert batch["src_wav"].shape == prev_shape, (
            batch["src_wav"].shape,
            prev_shape,
        )
