...
class VIDA(BaseAVModel):
    def __init__(self, args):
        ...
        self.configure_loss()
        self.metrics = speechmetrics.load(
            ["stoi", "sisdr", "bsseval", "mosnet"], window=None
        )
    ...
    def dereverb(self, batch, batch_idx):
        ...
        if self.args.decode_wav:
            ...
            pred = torch.cat(
                [torch.exp(pred[:, 0:1, :, :].clone()) - 1, pred[:, 1:2, :, :]], dim=1
            )
            ...
            pred = griffinlim(
                full_spec[:, 0, ...],
                full_spec[:, 1, ...],
                torch.hamming_window(400, device=full_spec.device),
                n_fft=512,
                hop_length=160,
                win_length=400,
                power=1,
...
class Predictor(nn.Module):
    def __init__(
        ...
        if self.use_visual:
            if use_rgb:
                self.rgb_net = VisualNet(
                    torchvision.models.resnet18(pretrained=True), 3
                )
            if use_depth:
                self.depth_net = VisualNet(
                    torchvision.models.resnet18(pretrained=True), 1
                )
    ...
    def forward(self, inputs):
        ...
        if self.mean_pool_visual:
            ...
            concat_visual_features = concat_visual_features.view(
                concat_visual_features.shape[0], -1, 1, 1
            )

        pred_mask, audio_feat = self.audio_net(
            inputs["spectrograms"], concat_visual_features, inputs["distance"]
        )
...
class AudioNet(nn.Module):
    def __init__(
        ...
        self.audionet_upconvlayer1 = unet_upconv(
            num_intermediate_feature, ngf * 8
        ...
        if self.no_mask:
            ...
            self.audionet_upconvlayer5 = unet_upconv(
                ngf * 2, output_nc, outermost=True, use_sigmoid=True
            )
