...
def load_rt60_estimator(device, path):
    """
    ...
    pretrained_weights = os.path.join(os.path.dirname(path), "rt60_estimator.pth")
    rt60_estimator.load_state_dict(
        torch.load(pretrained_weights, map_location="cpu")["predictor"]
    )
...
class BaseAVModel(pl.LightningModule):
        ...
        if self.args.decode_wav:
            if self.args.multires_stft:
                self.loss = MultiResolutionSTFTLoss(
                    fft_sizes=[int(x) for x in self.args.fft_sizes.split(",")],
                ...
                self.loss = LogMagSTFTLoss(
                    fft_size=512,
                    shift_size=self.args.hop_length,
                    win_length=400,
    ...
    def training_epoch_end(self, outputs):
        ...
        for metric in metrics:
            ...
            self.logger.experiment.add_scalar(
                f"train/{metric}", avg_value, self.current_epoch
            )
    ...
    def spec2wav(self, spec):
        if self.args.predict_mask:
            ...
            wav = GriffinLim(
                ...
                power=1,
                rand_init=False,
            )(spec.squeeze(1).cpu().permute(0, 2, 1)).to(device=self.device)
        return wav
    ...
    def validation_step(self, batch, batch_idx, test=False):
        stats, pred, tgt = self.run_step(
            batch, batch_idx, phase="val" if not test else "test"
        )
        ...
        if self.args.eval_input:
            pred_wav = (
                batch["src_wav"] if self.args.acoustic_matching else batch["recv_wav"]
            )
        else:
        ...
        if self.args.eval_speech_rt60:
            ...
            if RT60_ESTIMATOR is None:
                RT60_ESTIMATOR = load_rt60_estimator(
                    self.device, path=self.args.model_dir
                )
        ...
        if "log_mag_distance" in self.metrics:
            pred_spec, tgt_spec = eval_mag(pred_wav, log=True), eval_mag(
                tgt_wav, log=True
            )
        ...
        for i in range(tgt_wav.shape[0]):
            ...
            if len(self.speech_metric_names) != 0:
                ...
                for metric_name in scores:
                    ...
                    metric_scores[metric_name + "_diff"].append(
                        abs(scores[metric_name] - tgt_scores[metric_name])
                    )
                    metric_scores[metric_name + "_rel"].append(
                        abs(scores[metric_name] - tgt_scores[metric_name])
                        / tgt_scores[metric_name]
                    )
    ...
    def validation_epoch_end(self, outputs):
        ...
        for metric in metrics:
            avg_value = torch.stack(
                [output[metric] for output in gathered_outputs], dim=0
            ).mean()
            self.logger.experiment.add_scalar(
                f"val/{metric}", avg_value, self.current_epoch
            )
            output_str += f"{metric}: {avg_value:.4f}, "
            ...
            if metric == "mag_loss" and avg_value < self.best_val_loss:
                ...
                if self.global_rank == 0:
                    ...
                    torch.save(
                        {"state_dict": self.state_dict(), "hparams": self.args},
                        ckpt_path,
                    )
    ...
    def save_test_stats(self):
        ...
        file_path = os.path.join(
            self.args.model_dir, self.args.version, "test_stats" + postfix + ".json"
        )
