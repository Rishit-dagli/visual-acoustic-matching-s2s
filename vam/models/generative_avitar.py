...
from vam.models.generative_model import (
...
class GenerativeAViTAr(AViTAr):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_g_hat_mel = mel_spectrogram(
            y_g_hat.squeeze(1), 1024, 80, 160000, 256, 1024, 0, None
        )
        ...
        loss_mel = (
            F.l1_loss(y_mel, y_g_hat_mel) * self.args.stft_loss_weight
            if not self.args.remove_mel_loss
            else 0
        )
        ...
        loss_fm_f = (
            feature_loss(fmap_f_r, fmap_f_g) if not self.args.remove_fm_loss else 0
        )
        loss_fm_s = (
            feature_loss(fmap_s_r, fmap_s_g) if not self.args.remove_fm_loss else 0
        )
    ...
    def configure_optimizers(self):
        ...
        optim_g = torch.optim.AdamW(
            generator_params, self.args.generator_lr, betas=[0.8, 0.99]
        )
        optim_d = torch.optim.AdamW(
            itertools.chain(self.msd.parameters(), self.mpd.parameters()),
            self.args.discriminator_lr,
            betas=[0.8, 0.99],
        )
