...
parser.add_argument(
    "--use-pretrained-rt60-predictor", default=False, action="store_true"
)
...
def main():
    ...
    if args.slurm:
        ...
        executor.update_parameters(
            slurm_job_name=args.version,
            timeout_min=60 * 72,
            slurm_partition=args.part,
            nodes=args.num_node,
            gpus_per_node=args.n_gpus,
            cpus_per_task=10,
            slurm_constraint="volta32gb" if args.gpu_mem32 else None,
            slurm_mem=70 * 1024,
...
def run(args):
    ...
    if args.test or args.visualize:
        test_set = dataset(
            ...
            use_librispeech=args.use_librispeech,
            convolve_random_rir=args.convolve_random_rir,
            use_da=args.use_da,
            read_mp4=args.read_mp4,
        )
        test_dataset = torch.utils.data.DataLoader(
            test_set, num_workers=10, batch_size=args.batch_size, pin_memory=True
        )
    else:
        train_set = dataset(
            split="train",
    ...
    # Model training
    logger = loggers.TensorBoardLogger(args.model_dir, version=args.version)
    ...
    if args.eval_last or (args.auto_resume and not args.test):
        existing_checkpoints = sorted(
            glob.glob(os.path.join(args.model_dir, args.version, f"vam_epoch=*.ckpt"))
        )
    ...
    if args.eval_ckpt != -1:
        args.from_pretrained = os.path.join(
            args.model_dir, args.version, f"vam_epoch={args.eval_ckpt:04}.ckpt"
        )
        print(args.from_pretrained)
...
def generate_qual(args, model, dataset):
    ...
    for i, batch in enumerate(dataset):
        ...
        input_wav = (
            batch["original_src_wav"] if args.acoustic_matching else batch["recv_wav"]
        )
        ...
        input_spec = torch.stft(
            ...
            hop_length=160,
            win_length=400,
            window=torch.hamming_window(400, device=pred_wav.device),
            pad_mode="constant",
            center=True,
            return_complex=True,
        ).abs()
        ...
        plots_dir = os.path.join(
            args.model_dir,
            args.version,
            f"plots_{args.test_split}{'_clean' if args.eval_clean else ''}",
        )
        audio_dir = os.path.join(
            args.model_dir,
            args.version,
            f"audio_{args.test_split}{'_clean' if args.eval_clean else ''}",
        )
        image_dir = os.path.join(
            args.model_dir, args.version, f"images_{args.test_split}"
        )
        ...
        for j in tqdm(range(pred_wav.shape[0])):
            ...
            sf.write(
                os.path.join(audio_dir, f"{count}-tgt.wav"),
                tgt_wav[j],
                samplerate=16000,
            )
            sf.write(
                os.path.join(audio_dir, f"{count}-pred.wav"),
                pred_wav[j],
                samplerate=16000,
            )
            sf.write(
                os.path.join(audio_dir, f"{count}-input.wav"),
                input_wav[j],
                samplerate=16000,
            )
            count += 1
